//! Bitmap Font - 8x16 pixel font with ASCII character support
//!
//! Each character is represented as 16 bytes (one byte per row, 8 bits per column)
...
impl BitmapFont {
    /// Get glyph data for a character (returns 16 bytes for 8x16 bitmap)
    pub fn get_glyph(ch: char) -> &'static [u8; 16] {
        let idx = ch as usize;
        if idx >= 32 && idx < 128 {
            &FONT_DATA[idx - 32]
        } else {
            &FONT_DATA[0] // Default to space for unknown chars
        }
    }
...
/// 8x16 font data for ASCII 32-127 (96 characters)
/// Each character is 16 bytes, one byte per row
#[rustfmt::skip]
static FONT_DATA: [[u8; 16]; 96] = [
    // Space (32)
