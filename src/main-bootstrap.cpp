...
// =============================================================================
#include <void_engine/widget/widget_module.hpp>
...
int main(int argc, char** argv) {
    ...
    // - WidgetStateCore owns all widget state (registry, layout, style, etc.)
    // - Widget plugins read state through IWidgetAPI
    // - Widget plugins render UI elements
    // - State survives widget plugin hot-reloads
    spdlog::info("Phase 10: Widget System");

    // -------------------------------------------------------------------------
    // WIDGET STATE CORE - Owns all persistent widget state
    // -------------------------------------------------------------------------
    spdlog::info("  [widget_state_core]");

    void_widget::WidgetStateCoreConfig widget_config;
    widget_config.max_widgets = 10000;
    widget_config.max_layers = 64;
    widget_config.enable_hot_reload = true;
    widget_config.validate_commands = true;

    void_widget::WidgetStateCore widget_state(widget_config);
    widget_state.initialize();

    spdlog::info("    State stores initialized (Registry, Layout, Style, Animation, Binding)");
    spdlog::info("    Command processor ready");
    spdlog::info("    Render state ready");

    // -------------------------------------------------------------------------
    // THEMES (registered automatically in initialize())
    // -------------------------------------------------------------------------
    spdlog::info("  [themes]");

    // Default themes (dark, light, high_contrast) are registered during initialize()
    // Apply the dark theme by default
    widget_state.apply_theme("dark");

    spdlog::info("    Themes available: dark, light, high_contrast");
    spdlog::info("    Active theme: dark");
    ...
    auto* widget_api = widget_state.widget_api();
    auto hud_layer = widget_api->create_layer("hud", 100);
    auto menu_layer = widget_api->create_layer("menu", 200);
    auto overlay_layer = widget_api->create_layer("overlay", 300);

    spdlog::info("    Created layers: hud (z=100), menu (z=200), overlay (z=300)");
    ...
    // Create a test panel
    auto panel_id = widget_api->create_widget("panel", "main_panel");
    widget_api->set_position(panel_id, void_widget::Vec2{50, 50});
    widget_api->set_size(panel_id, void_widget::Vec2{300, 200});
    widget_api->set_layer(panel_id, hud_layer);

    // Create a test button inside the panel
    auto button_id = widget_api->create_widget("button", "test_button");
    widget_api->set_position(button_id, void_widget::Vec2{20, 20});
    widget_api->set_size(button_id, void_widget::Vec2{120, 40});
    widget_api->set_parent(button_id, panel_id);
    widget_api->set_text(button_id, "Click Me");

    // Create a health bar bound to game state
    auto health_bar_id = widget_api->create_widget("progress_bar", "health_bar");
    widget_api->set_position(health_bar_id, void_widget::Vec2{20, 80});
    widget_api->set_size(health_bar_id, void_widget::Vec2{260, 24});
    widget_api->set_parent(health_bar_id, panel_id);
    widget_api->set_value(health_bar_id, 1.0f);  // 100%

    // Bind health bar to player vitals from Phase 9
    widget_api->bind(health_bar_id, "value", "player.vitals.normalized_health",
                     void_widget::BindingMode::OneWay);

    spdlog::info("    Created widgets: main_panel, test_button, health_bar");
    spdlog::info("    Widget count: {}", widget_state.widget_registry().widgets.size());

    // -------------------------------------------------------------------------
    // WIDGET PLUGIN LOADING
    // -------------------------------------------------------------------------
    spdlog::info("  [widget_plugins]");

    // Configure widget plugin watcher
    void_widget::WidgetWatcherConfig widget_watcher_config;
    widget_watcher_config.watch_paths = {
        std::filesystem::current_path() / "widgets"
    };
    widget_watcher_config.auto_load_new = true;
    widget_watcher_config.auto_reload_changed = true;
    widget_watcher_config.debounce_time = std::chrono::milliseconds(500);
    widget_watcher_config.poll_interval = std::chrono::milliseconds(100);

    widget_state.configure_watcher(widget_watcher_config);

    // Load core widget plugin if it exists
    std::filesystem::path widget_plugin_path = std::filesystem::current_path() / "widgets" /
        (std::string("core_widget") + void_widget::native_widget_extension());

    if (std::filesystem::exists(widget_plugin_path)) {
        spdlog::info("    Found widget plugin: {}", widget_plugin_path.filename().string());

        if (widget_state.load_widget_plugin(widget_plugin_path)) {
            spdlog::info("    Loaded widget plugin: core_widget");
            spdlog::info("    Active widget plugins: {}", widget_state.active_plugin_count());
        } else {
            spdlog::warn("    Failed to load widget plugin: core_widget");
        }
    } else {
        spdlog::info("    No core_widget plugin found at: {}", widget_plugin_path.string());
        spdlog::info("    Build with: cmake --build build --target core_widget");
    }

    // Start watching for widget plugin changes
    widget_state.start_watching();
    spdlog::info("    Watching: {}", widget_watcher_config.watch_paths[0].string());
    spdlog::info("    Hot-reload: ENABLED");

    // -------------------------------------------------------------------------
    // CONNECT WIDGET SYSTEM TO GAMESTATE
    // -------------------------------------------------------------------------
    spdlog::info("  [gamestate_binding]");

    // Register game state as data source for widget bindings
    widget_state.register_data_source("player", [&game_state](const std::string& path) -> std::any {
        // Parse path like "vitals.normalized_health"
        if (path == "vitals.normalized_health") {
            void_plugin_api::EntityId player_id{1};
            const auto& combat = game_state.combat_state();
            auto it = combat.entity_vitals.find(player_id);
            if (it != combat.entity_vitals.end()) {
                return it->second.current_health / it->second.max_health;
            }
            return 1.0f;
        }
        if (path == "vitals.current_health") {
            void_plugin_api::EntityId player_id{1};
            const auto& combat = game_state.combat_state();
            auto it = combat.entity_vitals.find(player_id);
            if (it != combat.entity_vitals.end()) {
        ...
        if (path == "vitals.max_health") {
            void_plugin_api::EntityId player_id{1};
            const auto& combat = game_state.combat_state();
            auto it = combat.entity_vitals.find(player_id);
            if (it != combat.entity_vitals.end()) {
                return it->second.max_health;
            }
            return 100.0f;
        }
        return std::any{};
    ...
    spdlog::info("    GameStateCore data source registered");
    spdlog::info("    Data bindings active: player.vitals.*");

    // Perform one widget system update
    widget_state.update(1.0f / 60.0f);

    spdlog::info("Phase 10 complete");
    ...
    // Shutdown widget system first (stops widget watcher, unloads widget plugins)
    spdlog::info("  [widget-shutdown]");
    widget_state.shutdown();
    spdlog::info("    WidgetStateCore: shutdown complete (widget plugins unloaded, watcher stopped)");

    // Shutdown gameplay systems (stops plugin watcher, unloads plugins)
